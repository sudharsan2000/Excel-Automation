...
def main():
    ...
    for l,k,m,n in zip(INT_MARKS_PER_QUESTION,INT_MARKS,ENDSEM_MARKS_PER_QUESTION, ENDSEM_MARKS) :
        ...
        INT_NUM_GRADES['3'] = np.append([np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan],INT_NUM_GRADES['3'])
        INT_NUM_GRADES['2'] = np.append([np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan],INT_NUM_GRADES['2'])
        INT_NUM_GRADES['1'] = np.append([np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan],INT_NUM_GRADES['1'])
        INT_AVERAGE_GRADE = np.append([np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan],INT_AVERAGE_GRADE)
...
###############################################################################################################################3
        ...
        ENDSEM_NUM_GRADES['3'] = np.append([np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan],ENDSEM_NUM_GRADES['3'])
        ENDSEM_NUM_GRADES['2'] = np.append([np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan],ENDSEM_NUM_GRADES['2'])
        ENDSEM_NUM_GRADES['1'] = np.append([np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan],ENDSEM_NUM_GRADES['1'])
        ENDSEM_AVERAGE_GRADE = np.append([np.nan,np.nan,np.nan,np.nan,np.nan,np.nan,np.nan],ENDSEM_AVERAGE_GRADE)
    ...
    for key in COS:
        COS[key][INT_COS_count[key]+ ENDSEM_COS_count[key] + 2][6] = 'INT Total obtained'
        COS[key][INT_COS_count[key]+ ENDSEM_COS_count[key] + 3][6] = 'INT Total Attempted'
        COS[key][INT_COS_count[key]+ ENDSEM_COS_count[key] + 4][6] = 'INT Percentage'
        COS[key][INT_COS_count[key]+ ENDSEM_COS_count[key] + 5][6] = 'INT Grades on\nscale of 3'
        ...
        COS[key][INT_COS_count[key]+ ENDSEM_COS_count[key] + 8][6] = 'END Percentage'
        COS[key][INT_COS_count[key]+ ENDSEM_COS_count[key] + 9][6] = 'END Grades on\nscale of 3'
        ...
        COS[key][INT_COS_count[key]+ ENDSEM_COS_count[key] + 15][14] = 'Number of Students:'
        COS[key][INT_COS_count[key]+ ENDSEM_COS_count[key] + 16][14] = NUM_STUDENTS

        COS[key][INT_COS_count[key]+ ENDSEM_COS_count[key] + 13][6] = 'Total INT\nGrade 3'
        COS[key][INT_COS_count[key]+ ENDSEM_COS_count[key] + 14][6] = 'Total INT\nGrade 2'
        COS[key][INT_COS_count[key]+ ENDSEM_COS_count[key] + 15][6] = 'Total INT\nGrade 1'
        COS[key][INT_COS_count[key]+ ENDSEM_COS_count[key] + 16][6] = 'INT\n Avg Grade'

        COS[key][INT_COS_count[key]+ ENDSEM_COS_count[key] + 17][6] = 'Total END\nGrade 3'
        COS[key][INT_COS_count[key]+ ENDSEM_COS_count[key] + 18][6] = 'Total END\nGrade 2'
        COS[key][INT_COS_count[key]+ ENDSEM_COS_count[key] + 19][6] = 'Total END\nGrade 1'
        COS[key][INT_COS_count[key]+ ENDSEM_COS_count[key] + 20][6] = 'END\n Avg Grade'
