...
def finishSummary():
    ...
    for j in range(len(CO_percentage)):
        ...
        if(str(CO_SUMMARY[startColumn][startRow + 4 + j*7]) != 'NA'):
            CO_SUMMARY[startColumn-1][startRow + j + 6 + (iter) * 7] = str(CO_SUMMARY[startColumn][startRow + 4 + j*7]) + ' Percentage of Students secured ' + str(GRADE_RANGES[0]) + '% and above '
        if(str(CO_SUMMARY[startColumn+5][startRow + 4 + j*7]) != 'NA'):
            CO_SUMMARY[startColumn+7][startRow + j + 6 + (iter) * 7] = str(CO_SUMMARY[startColumn+5][startRow + 4 + j*7]) + ' Percentage of Students secured ' + str(GRADE_RANGES[0]) + '% and above '
