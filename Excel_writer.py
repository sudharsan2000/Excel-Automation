...
def writeToFile(path,original,obj,trans=1):
    ...
    for key in obj:
        ...
        if(key == 'CO Summary' or key == 'Percentage of POs and PSOs'):
            if (trans):
...
def calculatePOS():
    ...
    a = np.append([np.nan, np.nan],INTS[0].iloc[6:, 1])
    POS_write.append(a)
    a = np.append([np.nan, np.nan],INTS[0].iloc[6:, 2])
    POS_write.append(a)
...
def initSummary():
    ...
    CO_SUMMARY[startColumn][startRow + 5 +
                                        (iter ) * 7] = 'CO Attainments Cumulative'
    CO_SUMMARY[startColumn+8][startRow + 5 +
                                        (iter ) * 7] = 'CO Attainments Endsemester'
def finishSummary():
    ...
    for j in range(len(CO_percentage)):

        if(str(CO_SUMMARY[startColumn][startRow + 4 + j*7]) != 'NA'):
            CO_SUMMARY[startColumn-1][startRow + j + 6 + (iter) * 7] = 'CO' + str(j+1) + ' ' + str(CO_SUMMARY[startColumn][startRow + 4 + j*7]) + ' Percentage of Students secured ' + str(GRADE_RANGES[0]) + '% and above '
        if(str(CO_SUMMARY[startColumn+5][startRow + 4 + j*7]) != 'NA'):
            CO_SUMMARY[startColumn+7][startRow + j + 6 + (iter) * 7] = 'CO' + str(j+1) + ' ' + str(CO_SUMMARY[startColumn+5][startRow + 4 + j*7]) + ' Percentage of Students secured ' + str(GRADE_RANGES[0]) + '% and above '
...
def main(OP_FILE):
    ...
    for l, k, m, n in zip(INT_MARKS_PER_QUESTION, INT_MARKS, ENDSEM_MARKS_PER_QUESTION, ENDSEM_MARKS):
        ...
        INT_TARGET = ['Yes' if (g==3) else 'No' for g in GRADES] # [g==3 for g in GRADES] 
        ...
        if(INT_COS_count['CO' + str(iter)] != 0):
            ...
            appendtoframe(COS['CO' + str(iter)], INT_TARGET)
        else:
            for i in range(4):
        ...
        ENDSEM_TARGET = ['Yes' if (g==3) else 'No' for g in GRADES] # [g==3 for g in GRADES] 
        ...
        GRADES = np.append([np.nan, np.nan, np.nan, np.nan,
                            np.nan, np.nan, np.nan], GRADES)
        # ENDSEM_TARGET = np.append([np.nan, np.nan, np.nan, np.nan,
        #                     np.nan, np.nan, np.nan], ENDSEM_TARGET)
        ENDSEM_TARGET = np.append(['', '', '', '',
                              '', '', ''], ENDSEM_TARGET)
        ...
        if(ENDSEM_COS_count['CO' + str(iter)] != 0):
            ...
            appendtoframe(COS['CO' + str(iter)], PERCENTAGE)
            appendtoframe(COS['CO' + str(iter)], GRADES)
            appendtoframe(COS['CO' + str(iter)], ENDSEM_TARGET)
    ...
    for key in COS:
        if(key != 'CO Summary'):
            ...
            COS[key][INT_COS_count[key] +
                     ENDSEM_COS_count[key] + 6][6] = 'Target Attainment- Cumulative'

            COS[key][INT_COS_count[key] +
                     ENDSEM_COS_count[key] + 7][5] = 'END SEMESTER ONLY'
            COS[key][INT_COS_count[key] +
                     ENDSEM_COS_count[key] + 7][6] = 'Obtained'
            COS[key][INT_COS_count[key] +
                     ENDSEM_COS_count[key] + 8][6] = 'Attempted'
            COS[key][INT_COS_count[key] +
                     ENDSEM_COS_count[key] + 9][6] = 'Percentage'
            COS[key][INT_COS_count[key] + ENDSEM_COS_count[key] +
                     10][6] = 'Grades'

            COS[key][INT_COS_count[key] +
                     ENDSEM_COS_count[key] + 11][6] = 'Target Attainment- End Semester'

    calculatePOS()       
    CO_SUMMARY[8][0] = NUM_STUDENTS
    finishSummary()
    COS['CO Summary'] = CO_SUMMARY
    COS['Percentage of POs and PSOs'] = POS_write
